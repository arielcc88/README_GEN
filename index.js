...
function writeToFile(fileName, data) {
  ...
  Object.keys(data).forEach((section, position) => {
    switch (section) {
      case "Title":
        ...
        //adding badge
        tempStreamLoader += `![${data["License"]}](https://img.shields.io/static/v1?label=License&message=${data["License"]}&color=green)\n\n`;
        break;
  ...
  fsTream.end();
  console.log(
    `---- README for ${data.Title} has been successfully created. Happy Coding!! ----`
  );
}
...
function IsThereAReadme(fileName) {
  fileName = fileName || "README_N.md"; // setting to README.md if filename not passed.
  try {
...
function init() {
  ...
  inquirer.prompt(questions).then(function (answers) {
    writeToFile("README_N.md", answers);
    //console.log(answers);
  });
}
